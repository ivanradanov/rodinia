...
template<int BLOCK_SIZE>
void MatrixMulCUDAWrapper(dim3 grid, dim3 threads, float *d_C, float *d_A, float *d_B, int ax, int bx) {
  MatrixMulCUDA<BLOCK_SIZE> <<<grid, threads>>>(d_C, d_A, d_B, ax, bx);
}
/**
...
int MatrixMultiply(int argc, char **argv, int block_size, const dim3 &dimsA,
  ...
  if (block_size == 16) {
	  //MatrixMulCUDAWrapper<16>(grid, threads, d_C, d_A, d_B, dimsA.x, dimsB.x);
	  MatrixMulCUDA<16> <<<grid, threads>>>(d_C, d_A, d_B, dimsA.x, dimsB.x);
  } else {
	  //MatrixMulCUDAWrapper<32>(grid, threads, d_C, d_A, d_B, dimsA.x, dimsB.x);
