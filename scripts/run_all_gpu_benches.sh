...
for target in $TARGETS; do
  ...
  if [ "$PGO_PROF" == "1" ]; then
    for i in $PGO_CONFIGS; do
      ...
      MAX_ALTS=$(POLYGEIST_GPU_ALTERNATIVES_PRINT_INFO=1 make POLYGEIST_ALTERNATIVES_MODE=pgo_prof CONFIG="$i" TARGET=$target MY_VERIFICATION_DISABLE=1 cuda -kjO 2>&1 | tee "$COMPILATION_LOG" | grep -E "Generated ([0-9]+) alternatives" | sed --expression='s/Generated //' | sed --expression='s/ alternatives//' | sort -n | tail -1)
  ...
  if [ "$PGO_OPT" == "1" ]; then
    for i in $PGO_CONFIGS; do
      ...
      POLYGEIST_PGO_DATA_DIR="$PGO_DIR" \
        make POLYGEIST_ALTERNATIVES_MODE=pgo_opt CONFIG="$i" TARGET=$target MY_VERIFICATION_DISABLE=1 cuda -kjO &> "$COMPILATION_LOG"
