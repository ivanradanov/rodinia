...
int main(int argc, char* argv[]) {
  ...
  for (int p = 1; p <= repeat; p++) {
    ...
    auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda dropout-cuda main.cu,0);
    ...
    auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda dropout-cuda main.cu,0);
    total_time += time;
