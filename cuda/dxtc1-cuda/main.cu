...
__global__
void compress (const uint* permutations, 
               const uint* image, 
               uint2* result, 
               float* alphaTable4, 
               int* prods4,
...
int main(int argc, char** argv) 
{
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda dxtc1-cuda main.cu,0);
