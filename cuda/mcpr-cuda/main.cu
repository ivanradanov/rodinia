...
int main(int argc, char* argv[]) {
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda mcpr-cuda main.cu,0);
  ...
  time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda mcpr-cuda main.cu,1);
  printf("Average kernel execution time: %f (s)\n", (time * 1e-9f) / repeat);
  ...
  time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda mcpr-cuda main.cu,2);
  printf("Average kernel execution time: %f (s)\n", (time * 1e-9f) / repeat);
