...
void distance_device(const double4* loc, double* dist, const int n, const int iteration) {
  ...
  auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda haversine-cuda distance.cu,0);
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda haversine-cuda distance.cu,0);
