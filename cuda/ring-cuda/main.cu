...
int main(int argc, char* argv[])
{
  ...
  for (long len = min_len; len <= max_len; len = len * 4) {
    ...
    for (int n = 0; n < repeat; n++) {
      ...
      cudaDeviceSynchronize(); // wait for memory copies to finish 
    }

    auto end = std::chrono::steady_clock::now();
    auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda ring-cuda main.cu,0);
