...
int main() {
  ...
  //------------------------------------------------------------------------------------
  for (auto i = 0; i < TOTAL_RUN_COUNT; ++i) {
    auto start=std::chrono::high_resolution_clock::now();MY_START_CLOCK(cuda dispatch-cuda main.cu,1);
    EmptyKernel<<<dim3(NUM_GROUPS), dim3(GROUP_SIZE)>>>();
  ...
  //------------------------------------------------------------------------------------
  for (auto i = 0; i < TOTAL_RUN_COUNT; ++i) {
    auto start=std::chrono::high_resolution_clock::now();MY_START_CLOCK(cuda dispatch-cuda main.cu,2);
