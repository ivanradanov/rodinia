  ...
  int64_t* mask, gscalar_t* grad_predict)
{
  ...
  auto start=std::chrono::high_resolution_clock::now();MY_START_CLOCK(cuda crossEntropy-cuda main.cu,0);
...
void LossNLL_BWD(int iterations) {
  ...
  auto start=std::chrono::high_resolution_clock::now();MY_START_CLOCK(cuda crossEntropy-cuda main.cu,1);
