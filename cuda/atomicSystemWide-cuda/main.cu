...
int main(int argc, char **argv)
{
  ...
  else
  {
    ...
    cudaMallocManaged((void**)&atom_arr, sizeof(int)*numData);
  }
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda atomicSystemWide-cuda main.cu,0);
