...
int main (int argc, char *argv[]) {
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda mriQ-cuda main.cu,0);
  ...
  time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda mriQ-cuda main.cu,1);
