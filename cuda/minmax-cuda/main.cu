...
void eval (const T bounding_box_size, const int repeat) {
  ...
  auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda minmax-cuda main.cu,0);
  ...
  start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda minmax-cuda main.cu,1);
