...
#define h_Vm(x,y,z) h_Vm[ z + nz * ( y + ny * ( x  ) ) ]
  ...
  auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda stencil3d-cuda main.cu,0);
  ...
  auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda stencil3d-cuda main.cu,0);
