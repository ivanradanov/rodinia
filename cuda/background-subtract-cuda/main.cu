...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < repeat; i++) {
    ...
    if (i >= 2) {
      if (merged) {
MY_START_CLOCK(background-subtract-cuda main.cu,0);
        merge <<< grids, blocks >>> ( imgSize, d_Img, d_Img1, d_Img2, d_Tn, d_Bn );
        cudaDeviceSynchronize();
        auto end = std::chrono::steady_clock::now();
MY_STOP_CLOCK(background-subtract-cuda main.cu,0);
      }
      else {
MY_START_CLOCK(background-subtract-cuda main.cu,1);
        ...
        cudaDeviceSynchronize();
        auto end = std::chrono::steady_clock::now();
MY_STOP_CLOCK(background-subtract-cuda main.cu,1);
      }
    }
  }
