...
// ****************************************************************************
__global__
void md5hash_kernel(int *foundIndex, unsigned char *foundKey, unsigned int *foundDigest, 
		    int keyspace,
		    const int byteLength,
		    const int valsPerByte,
...
int main(int argc, char** argv) 
{
    ...
    for (int size = 1; size <= 4; size++) {
      ...
      for (int pass = 0 ; pass < passes ; ++pass)
      {
          ...
          auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda md5hash-cuda MD5Hash.cpp,0);
          if (offload == 0)
          {
