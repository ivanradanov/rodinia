...
int main(int argc, char** argv) 
{
  ...
  for (int size = 1; size <= 4; size++) {
    ...
    for (int pass = 0 ; pass < passes ; ++pass)
    {
      ...
      auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda md5hash-cuda MD5Hash.cu,0);
      if (offload == 0)
      {
      ...
      auto t = std::chrono:: duration_cast<std::chrono::milliseconds>(end - start).count();MY_STOP_CLOCK(cuda md5hash-cuda MD5Hash.cu,0);
