...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    auto start=std::chrono::steady_clock::now();MY_START_CLOCK(cuda gpp-cuda main.cu,0);
    ...
    auto time = std::chrono:: duration_cast<std::chrono::nanoseconds>(end - start).count();MY_STOP_CLOCK(cuda gpp-cuda main.cu,0);
    total_time += time;
  }
