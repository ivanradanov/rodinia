                              ...
                              fp ***y, fp **x, fp **params, fp *com)
{
    ...
    // Select the first device (previously selected for the context) (if there are multiple devices, choose the first
    // one)
    cl_device_id device;
    ...
    cl_mem d_finavalu;
    d_finavalu = clCreateBuffer(context, CL_MEM_READ_WRITE, d_finavalu_mem, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_mem d_params;
    d_params = clCreateBuffer(context, CL_MEM_READ_WRITE, d_params_mem, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
